...
- **Editing the hook's `CONTEXT`:** it lives in a quoted heredoc in `hooks/bootstrap-project.sh` and `python3`'s `json.dumps` builds the payload, so the text needs no escaping — quotes, backslashes, backticks, `$VAR`, `$(...)` and newlines are all safe literals, and a malformed hook response is not possible. Three rules:
  - Reference the repo via the `{{REPO}}` / `{{REPO_OWNER}}` / `{{REPO_NAME}}` placeholders rather than shell variables — they are substituted at emit time, and a quoted heredoc will not expand a shell variable anyway.
  - Keep the heredoc **quoted** (`<<'EOF'`). Unquoting it reintroduces shell interpolation over prose full of `$`, backticks and `$(...)`.
  - Read it with `IFS= read -r -d '' CONTEXT <<'EOF' || true`. **Never assign it via `CONTEXT="$(cat <<'EOF' ... )"`** — that form is what the hook used until `ec47230`, and it does not parse under bash 3.2, which is what macOS ships as `/bin/bash` and what the shebang resolves to. bash 3.2 scans `$( )` for the closing paren while tracking quote state, without knowing a quoted heredoc body is literal, so a single apostrophe in the prose (`the user's task`) sent it past the closing paren and the hook died with a syntax error before emitting anything — silently, on every macOS session. `read` involves no command substitution, so the body can contain any character. Check with `/bin/bash -n hooks/bootstrap-project.sh`, not a newer bash.

  If `python3` is missing the hook exits silently rather than emit invalid JSON.
