...
2. If this repo has **no** `CLAUDE.md`: inspect the codebase (stack, package manager, build/test/lint commands, folder structure) and write `./CLAUDE.md` based on the template. Remove template sections irrelevant to this repo and add a project-specific section with what you discovered. **Git Workflow, Session naming and Compact Instructions are never irrelevant** — they are workflow rules that apply everywhere; carry them over verbatim.
3. If this repo **already has** a `CLAUDE.md`: merge instead. Keep all project-specific content, update/add the general sections from the template (including **Session naming**, which is newer than most existing `CLAUDE.md` files and will usually be missing), and remove anything the template has since dropped. Show a summary of what changed.
