...
# python3 builds the JSON payload below (see the emit step) so this text needs no
# escaping. It is a quoted heredoc, so nothing here is interpreted by the shell:
...
Step 1: fetch templates/CLAUDE.md from {{REPO}} using whichever transport works in this environment, in order:
  (a) gh CLI: gh api -H 'Accept: application/vnd.github.raw' repos/{{REPO}}/contents/templates/CLAUDE.md
...
Step 4: show the user both files for review and offer to commit them so other machines and cloud sessions pick them up.
...
# hook response is not possible regardless of what CONTEXT contains. install.sh
# already hard-requires python3; if it is missing anyway, stay silent rather than
# risk emitting invalid JSON into the session.
...
for token in ("REPO", "REPO_OWNER", "REPO_NAME"):
    context = context.replace("{{" + token + "}}", os.environ[token])
